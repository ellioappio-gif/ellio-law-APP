...
export const DiscoveryManagerScreen: React.FC<any> = () => {
  const [discoveries, setDiscoveries] = useState<Discovery[]>([
    { id: '1', title: 'Interrogatories', dueDate: '2024-02-01', completed: false },
  ]);
  ...
  const addDiscovery = () => {
    ...
    setDiscoveries([...discoveries, { id: Date.now().toString(), title, dueDate: date, completed: false }]);
    setTitle('');
    setDate('');
  };

  const toggleComplete = (id: string) => {
    setDiscoveries(discoveries.map(d => d.id === id ? { ...d, completed: !d.completed } : d));
  };

  const deleteDiscovery = (id: string) => {
    Alert.alert('Delete', 'Remove discovery?', [
      { text: 'Cancel' },
      { text: 'Delete', onPress: () => setDiscoveries(discoveries.filter(d => d.id !== id)), style: 'destructive' },
    ]);
  };

  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Discovery Requests</Text>

      <View style={styles.inputSection}>
        <TextInput style={styles.input} placeholder="Request type" value={title} onChangeText={setTitle} />
        <TextInput style={styles.input} placeholder="Due date (YYYY-MM-DD)" value={date} onChangeText={setDate} />
        <TouchableOpacity style={styles.button} onPress={addDiscovery}>
